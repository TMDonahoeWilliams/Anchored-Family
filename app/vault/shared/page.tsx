...
export default function VaultSharedPage() {
  ...
  async function upload() {
    ...
    const key = `${HOUSEHOLD_ID}/shared/${Date.now()}_${file.name}`;
  ...
  return (
    <div className="container">
      <h1 className="page-title">Family Vault — Shared</h1>
      ...
      <section className="card section">
        ...
        <div className="subcategories" style={{ marginBottom: 8 }}>
          ...
          />
          <button className="btn btn--sm accent-green" onClick={upload}>⬆️ Upload</button>
        </div>
        <div className="subcategories">
          <Link href="/vault" className="btn btn--sm">↩ Family Vault</Link>
          <Link href="/vault/locked" className="btn btn--sm accent-rose">🔐 Locked</Link>
        </div>
      </section>
