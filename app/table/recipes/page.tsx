...
export default function RecipesPage() {
    ...
    if (file) {
      try {
        ...
        if (upErr) {
          ...
          // Continue without image rather than failing the whole recipe
        } else {
  ...
  async function searchRecipes() {
    ...
    try {
      ...
      if (!r.ok) {
        if (j.error === 'Missing SPOONACULAR_API_KEY') {
          throw new Error('Spoonacular API key not configured. Please add SPOONACULAR_API_KEY to your environment variables.');
        }
      ...
      if (j.results?.length === 0) {
        alert('No recipes found. Try different search terms or adjust your filters.');
      }
    } catch (e: any) {
  ...
  async function saveFromSpoonacular(spoonId: number) {
    try {
      const r = await fetch(`/api/recipes?id=${spoonId}`);
      const j = await r.json();
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          summary: summary ? summary.replace(/<[^>]+>/g, '').slice(0, 500) : null,
          ingredients: ing.length ? ing : null,
          instructions: instructions || null,
      ...
      if (error) {
        ...
        return alert(`Failed to save recipe: ${error.message}`);
      }
      
      setRecipes(prev => [data as Recipe, ...prev]);
      alert(`"${title}" saved to My Cookbook!`);
    } catch (error) {
      console.error('Error saving recipe from Spoonacular:', error);
          ...
          <section className="card section">
            ...
            <div className="subcategories" style={{ marginBottom: 8 }}>
              <input 
                ...
                placeholder="Search (e.g., chicken tacos)" 
                value={q} 
              ...
              <input 
                className="btn btn--sm" 
                placeholder="Max minutes (e.g., 30)" 
                ...
                max="300"
              />
              <button className="btn btn--sm accent-blue" onClick={searchRecipes} disabled={loadingSearch || !q.trim()}>
          ...
          <section className="card section">
            ...
            {loadingSearch ? (
              ...
              <div className="subtitle">
                {q.trim() ? 'No recipes found. Try different search terms or adjust your filters.' : 'Enter a search term and click "Search" to find recipes.'}
              </div>
            ) : (
              <>
                <div className="subtitle" style={{ marginBottom: 16 }}>
                  Found {results.length} recipes for "{q}"
                  {diet && ` with ${diet} diet`}
                  {maxReadyMinutes && ` ready in ≤${maxReadyMinutes} minutes`}
                </div>
                <div className="categories">
                  {results.map((r:any) => (
                    <article key={r.id} className="category-card" style={{ flexDirection:'column', gap:8 }}>
                      ...
                      <div className="subcategories">
                        ...
                        <a className="btn btn--sm" href={r.sourceUrl || `https://spoonacular.com/recipes/${r.title.replace(/\s+/g, '-').toLowerCase()}-${r.id}`} target="_blank" rel="noopener noreferrer">View Original</a>
                      </div>
                    </article>
                  ))}
                </div>
